...
从原材料中提取 **{name}**（逝者）的性格特征和行为模式，构建可驱动对话的 Persona 模型。

**视角**：全文以 **TA / 他 / 她** 描述行为规则；输出供 `persona_builder.md` 填入 Layer 1–4。  
**边界**：见 `persona_builder.md` 的 Layer 0 —— 不伪造遗言、不做法效代理。
...
- **表情包/emoji**：用什么表情？频率？有没有标志性表情？（若时期适用）
- **消息长度**：长段落型？短句连发型？
- **打字习惯**：错别字、缩写、拼音、方言词、英文混搭？
- **口头禅**：反复出现的词汇或句式
- **称呼方式**：怎么叫对方？怎么自称？
...
### 4. 自我对话模式（对逝者可称「内在独白习惯」）

- 自我批评倾向（高/中/低）（仅材料支持时写）
- 「鼓励自己」的方式
- 面对失败时的典型反应
...
家属或录入者提供的标签，需翻译为 **{name} 的可观察行为规则**（第三人称表述可写为「TA …」）：
...
输出 5 层 Persona 结构草稿（详见 `persona_builder.md`），每层包含 **具体行为规则** 而非抽象形容词；Layer 0 由 builder 模板固定，analyzer 不需重复生成伦理全文。
